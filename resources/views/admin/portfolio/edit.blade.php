...
@section('title', 'Edit Portfolio - ' . setting('site_name', 'Andrew.Devlog') . ' Admin')
@section('header_title', 'Edit Project')
@section('header_subtitle', 'Perbarui detail karya digital favorit Anda.')
...
<div class="mb-8 flex items-center justify-between">
    ...
    <div class="px-4 py-2 bg-white/2 border border-white/5 rounded-full text-[10px] text-gray-500 font-bold uppercase tracking-widest leading-relaxed">
        ID: #{{ $portfolio->id }} / <span class="text-primary italic">{{ $portfolio->slug }}</span>
    </div>
</div>

<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.portfolio.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="bg-gray-900 shadow-2xl border border-white/5 rounded-[40px] p-10 space-y-10">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="category" id="category" required value="{{ old('category', $portfolio->category) }}"
            ...
            <div class="space-y-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
                    ...
                    @php
                        $isUrl = str_starts_with($portfolio->image, 'http');
                        $currentType = old('image_type', $isUrl ? 'url' : 'file');
                    @endphp

                    <!-- Toggle -->
                    <div class="flex p-1 bg-white/5 rounded-xl border border-white/10 w-fit">
                        <button type="button" onclick="setImageType('file')" id="btn-file" class="px-3 py-1.5 rounded-lg font-black text-[9px] uppercase tracking-widest transition-all {{ $currentType == 'file' ? 'bg-primary text-black' : 'text-gray-400 hover:text-white' }}">
                            FILE
                        </button>
                        <button type="button" onclick="setImageType('url')" id="btn-url" class="px-3 py-1.5 rounded-lg font-black text-[9px] uppercase tracking-widest transition-all {{ $currentType == 'url' ? 'bg-primary text-black' : 'text-gray-400 hover:text-white' }}">
                            URL
                        </button>
                    </div>
                    <input type="hidden" name="image_type" id="image_type" value="{{ $currentType }}">
                </div>

                <div class="relative group">
                    <!-- File Input Container -->
                    <div id="file-input-container" class="{{ $currentType == 'file' ? '' : 'hidden' }}">
                        <div class="w-full aspect-[4/3] bg-white/2 border-2 border-dashed border-white/5 rounded-[32px] flex flex-col items-center justify-center transition-all group-hover:bg-white/5 group-hover:border-primary/30 overflow-hidden relative cursor-pointer" onclick="document.getElementById('image').click()">
                            <img id="preview-file" src="{{ !$isUrl ? asset('storage/' . $portfolio->image) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ !$isUrl ? '' : 'hidden' }}">
                            <div id="placeholder-file" class="text-center flex flex-col items-center gap-3 {{ !$isUrl ? 'hidden' : '' }}">
                    ...
                    <!-- URL Input Container -->
                    <div id="url-input-container" class="{{ $currentType == 'url' ? '' : 'hidden' }} space-y-4">
                        <div class="relative group">
                            <input type="url" name="image_url" id="image_url" value="{{ old('image_url', $isUrl ? $portfolio->image : '') }}" placeholder="https://example.com/image.jpg"
                                class="w-full bg-white border border-white/5 text-gray-900 px-6 py-4 rounded-2xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none font-medium"
                                oninput="previewUrl(this.value)">
                        </div>
                        <!-- URL Preview -->
                        <div id="url-preview-container" class="{{ $isUrl || old('image_url') ? '' : 'hidden' }} border-2 border-dashed border-white/5 rounded-[32px] aspect-[4/3] overflow-hidden bg-white/2 relative">
                            <img id="preview-url" src="{{ $isUrl ? $portfolio->image : old('image_url') }}" class="absolute inset-0 w-full h-full object-cover" />
                        </div>
                    </div>
                </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" required rows="4"
                ...
                placeholder="Ceritakan sedikit tentang proyek ini, tantangan, dan solusi...">{{ old('description', $portfolio->description) }}</textarea>
            @error('description')
                <p class="mt-2 text-red-500 text-[10px] font-bold ml-1 uppercase">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center gap-4 bg-white/2 border border-white/5 p-6 rounded-3xl">
            <label class="relative inline-flex items-center cursor-pointer group">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" class="sr-only peer" {{ $portfolio->is_featured ? 'checked' : '' }}>
        ...
        <div class="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <a href="{{ route('admin.portfolio.index') }}" class="w-full bg-white/2 border border-white/5 hover:bg-white/5 text-gray-400 font-black py-5 rounded-[32px] transition-all flex items-center justify-center gap-3">
                <span>BATALKAN</span>
            </a>
        </div>
    </form>
