...
@section('title', 'Tambah Client - Admin Dashboard')
@section('header_title', 'Tambah Client Baru')
@section('header_subtitle', 'Masukkan detail perusahaan partner atau client Anda.')

@section('admin_content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white/5 backdrop-blur-xl rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl">
        <form action="{{ route('admin.clients.store') }}" method="POST" enctype="multipart/form-data" class="p-10 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Google, PHP, Laravel, dll." required
                ...
                <div class="space-y-3">
                    ...
                    <select name="type" required class="w-full bg-white/5 border border-white/10 text-white px-6 py-4 rounded-2xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all appearance-none cursor-pointer">
                        <option value="partner" {{ old('type', request('type')) == 'partner' ? 'selected' : '' }} class="bg-slate-900 text-white">Mitra Global</option>
                        <option value="tech" {{ old('type', request('type')) == 'tech' ? 'selected' : '' }} class="bg-slate-900 text-white">Bahasa Pemrograman (Tech Stack)</option>
                    </select>
                ...
                <div class="space-y-3 md:col-span-2 border-t border-white/5 pt-8">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        <label class="text-sm font-bold text-gray-400 uppercase tracking-widest px-1">Logo Perusahaan <span class="text-red-500">*</span></label>
                        
                        <!-- Toggle -->
                        <div class="flex p-1 bg-white/5 rounded-xl border border-white/10 w-fit">
                            <button type="button" onclick="setLogoType('file')" id="btn-file" class="px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all bg-primary text-black">
                                <i class="fas fa-upload mr-2"></i> Upload File
                            </button>
                            <button type="button" onclick="setLogoType('url')" id="btn-url" class="px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all text-gray-400 hover:text-white">
                                <i class="fas fa-link mr-2"></i> Link URL
                            </button>
                        </div>
                        <input type="hidden" name="logo_type" id="logo_type" value="{{ old('logo_type', 'file') }}">
                    </div>

                    <div class="relative group">
                        <!-- File Input Container -->
                        <div id="file-input-container" class="{{ old('logo_type', 'file') == 'file' ? '' : 'hidden' }}">
                            <label for="logo" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-white/10 rounded-3xl cursor-pointer bg-white/5 hover:bg-white/[0.08] hover:border-primary/30 transition-all group overflow-hidden">
                                <div id="preview-placeholder" class="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    <p class="mb-2 text-sm text-gray-400 font-bold tracking-tight">Ketik atau seret logo ke sini</p>
                                    <p class="text-xs text-gray-600">PNG, JPG, SVG, atau WebP (Maks. 2MB)</p>
                                </div>
                                <img id="logo-preview-file" class="absolute inset-0 w-full h-full object-contain hidden p-6 bg-white" />
                                <input id="logo" name="logo" type="file" class="hidden" accept="image/*" onchange="previewImage(this)" />
                            </label>
                        </div>

                        <!-- URL Input Container -->
                        <div id="url-input-container" class="{{ old('logo_type') == 'url' ? '' : 'hidden' }} space-y-4">
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none text-gray-600 group-focus-within:text-primary transition-colors">
                                    <i class="fas fa-image"></i>
                                </div>
                                <input type="url" name="logo_url" id="logo_url" value="{{ old('logo_url') }}" placeholder="https://example.com/logo.png"
                                    class="w-full bg-white/5 border border-white/10 text-white pl-14 pr-6 py-4 rounded-2xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all placeholder:text-gray-600"
                                    oninput="previewUrl(this.value)">
                            </div>
                            <!-- URL Preview -->
                            <div id="url-preview-container" class="hidden border-2 border-dashed border-white/10 rounded-3xl h-48 overflow-hidden bg-white p-6">
                                <img id="logo-preview-url" src="" class="w-full h-full object-contain" />
                            </div>
                        </div>
                    </div>
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center gap-4 bg-white/5 p-6 rounded-2xl border border-white/10">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-gray-500 text-xs mt-1">Tampilkan client ini di marquee beranda segera setelah disimpan.</p>
                        </div>
            ...
            <div class="flex items-center gap-4 pt-6">
                ...
                <button type="submit" class="flex-[2] px-8 py-4 bg-primary hover:bg-primary-dark text-white rounded-2xl font-bold transition-all shadow-xl shadow-primary/20 hover:-translate-y-1 active:scale-95">Simpan Client</button>
            </div>
        </form>
    </div>
</div>
