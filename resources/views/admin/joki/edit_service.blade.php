...
@section('title', 'Edit Layanan Joki - ' . setting('site_name', 'Andrew.Devlog') . ' Admin')
@section('header_title', 'Edit Layanan Joki')
@section('header_subtitle', 'Perbarui informasi layanan joki.')

@section('admin_content')
<div class="max-w-4xl">
    <form action="{{ route('admin.joki.update', $service) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-gray-900 border border-white/5 rounded-[40px] p-8 md:p-12 shadow-2xl space-y-10">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="title" value="{{ old('title', $service->title) }}" placeholder="e.g. Bantuan Tugas Koding" required
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $service->icon) }}" placeholder="e.g. fa-code" required
                        class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-primary outline-none font-bold text-white transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="price" value="{{ old('price', $service->price) }}" placeholder="e.g. Rp 10k"
                        class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-primary outline-none font-bold text-white transition-all">
                </div>
                <div class="space-y-3">
            ...
            <div class="space-y-3">
                ...
                <textarea name="desc" rows="4" placeholder="Jelaskan secara singkat apa yang ditawarkan layanan ini..." required
                    class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-primary outline-none font-bold text-white transition-all resize-none">{{ old('desc', $service->desc) }}</textarea>
            </div>
        </div>
