...
@section('title', 'Tambah Paket - ' . $service->title . ' - Admin')
@section('header_title', 'Tambah Paket Baru')
@section('header_subtitle', 'Layanan: ' . $service->title)

@section('admin_content')
<div class="max-w-4xl">
    <form action="{{ route('admin.joki.packages.store', $service) }}" method="POST" class="space-y-8">
        @csrf
        ...
        <div class="bg-gray-900 border border-white/5 rounded-[40px] p-8 md:p-12 shadow-2xl space-y-10">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Paket CRUD Santai" required
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" name="price" value="{{ old('price') }}" placeholder="e.g. Rp 1.000.000" required
                        class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-primary outline-none font-bold text-white transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" name="desc" value="{{ old('desc') }}" placeholder="e.g. Sistem administrasi data dasar..."
                        class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-primary outline-none font-bold text-white transition-all">
                </div>
            </div>
            ...
            <div class="space-y-3">
                ...
                <textarea name="features" rows="6" placeholder="Operasi CRUD Lengkap&#10;Validasi Form&#10;Database Ready..." required
                    class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:border-primary outline-none font-bold text-white transition-all resize-none">{{ old('features') }}</textarea>
